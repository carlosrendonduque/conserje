 ...
 * site, which is what makes the cache breakpoint in claude.ts actually hit --
...
export const TOOL_NAME = 'record_lead';

export function systemPrompt(site: SiteConfig): string {
  const collect = bulletList(site.collect);
  const bands = bulletList(site.budgetBands);

  return `You are the front-desk assistant for ${site.name}. You greet visitors on the
...
Call the \`${TOOL_NAME}\` tool exactly once, when any of these is true:
 ...
 *
 * `strict` is on so the arguments validate against this schema exactly -- the
 * payload goes straight into automation, where a missing field is a broken
 * workflow run rather than a cosmetic problem.
 */
export function leadTool(site: SiteConfig): Tool {
  return {
    name: TOOL_NAME,
    description:
      'Record the qualified visitor so a human can follow up. Call this exactly once per ' +
      'conversation, when you have enough to brief someone, or when the visitor asks to be contacted.',
    strict: true,
    input_schema: {
      type: 'object',
      ...
      properties: {
        need: {
          type: 'string',
          description:
            'What the visitor wants, in their own words, as specifically as they gave it. ' +
            'One or two sentences.',
        },
        summary: {
          type: 'string',
          description:
            'A short brief for the human who picks this up: the situation, what was asked ' +
            'for, and anything notable about fit.',
        },
        name: { type: ['string', 'null'], description: 'Visitor name if given, otherwise null.' },
        email: {
          type: ['string', 'null'],
          description:
            'Email address exactly as given, otherwise null. Never guess or reconstruct an address.',
        },
        phone: { type: ['string', 'null'], description: 'Phone number exactly as given, otherwise null.' },
        company: {
          type: ['string', 'null'],
        ...
        budgetBand: {
          ...
          // does not match declared type". The other nullable fields carry no
          // enum, so the plain union is still fine for them.
          anyOf: [{ type: 'string', enum: [...site.budgetBands] }, { type: 'null' }],
          description:
            'Exactly one budget band label from the system prompt, or null if budget never ' +
